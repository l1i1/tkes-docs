...
Tokeness 采用预充值、按调用消耗扣费的模式。模型价格会随上游政策和渠道变化调整，最终以 Tokeness 控制台、模型广场或正式报价为准。
...
| 价格单位 | 通常按每 1M tokens 的输入和输出价格展示 |
| 汇率假设 | 现有渠道报价表按 `1 USD = 7 CNY` 折算 |

## 渠道示例

本目录中的 `Tokeness_API_Pricing.xlsx` 显示了 2026-06 的渠道报价结构，包括：
