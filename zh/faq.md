...
部分工具会尝试自动拉取模型列表。Tokeness 的 `/v1/models` 只返回公开分组（`default`）的模型；Claude、GPT 等付费分组不在该公开清单里，自动发现会得到空列表。这类工具需要关闭自动发现、手动填写完整模型名。

## 客户端显示缓存命中率是 0 怎么办？

先确认两件事，再判断是平台问题还是客户端配置问题。

第一，看请求走的是哪种协议。`/v1/chat/completions`（OpenAI 兼容）与 `/v1/messages`（Anthropic 原生）都受支持，模型名和分组决定上游渠道。

第二，看客户端有没有发送缓存断点。Anthropic 的提示缓存是**显式**的：请求里必须带 `cache_control` 断点，上游才会缓存这段前缀。有些客户端（例如 Hermes Agent）只对它能识别的供应商和域名自动注入断点，对接自定义网关时默认不注入——这种情况下无论怎么切换 API 协议，命中率都会保持为 0，必须在客户端配置里显式开启。Hermes 的配置方式见 [Hermes Agent](/zh/integrations/hermes)。

在 Tokeness 控制台的**使用日志**里打开请求详情，查看 `缓存写入` 与 `缓存读取`：第一轮应看到写入大于 0，多轮对话的后续轮次应看到读取大于 0。如果每一轮的输入 token 都接近整段上下文、读取恒为 0，说明断点没有发出来。
